...
(defn update-checkpoint-atom! [block-number tx log-index]
  (log/info "Updating checkpoint atom" {:block-number block-number :log-index log-index})
  ...
  (if file-path
    (try
      ...
      (catch js/Error e (do
                          ...
                          (callback e nil))))
    (log/info (str "web3-events checkpoint file disabled: " file-path))))
...
(defn register-callback! [event-key callback & [callback-id]]
  (log/info (str "register-callback! for event: " event-key))
  (let [[contract-key event] (if-not (= event-key ::after-past-events-dummy-key)
    ...
    (swap! (:callbacks @web3-events) (fn [callbacks]
                                       (log/info ">>> register-callback! swap")
                                       (-> callbacks
...
(defn- start-dispatching-latest-events! [events last-block-number]
  (log/info ">>> start-dispatching-latest-events" @web3-events)
  (log/info ">>> start-dispatching " {:events events :last-block-number last-block-number})
  (let [event-filters (doall (for [[contract event->callbacks] (dissoc @(:callbacks @web3-events) :callback-id->path)
                                   [event callbacks] event->callbacks]
                               (do
                                 (log/info "smart-contracts/subscribe-events" {:event event :from-block last-block-nuumber})
  ...
  (web3-eth/connected?
    ...
    (fn [_err-connected listening?]
      (if-not listening?
        ...
        (load-checkpoint
          (fn [err checkpoint]
            (println ">>> district.server.web3-events/start INSIDE load-checkpoint callback" {:err err :checkpoint checkpoint})
            (web3-eth/get-block-number
              ...
              (fn [_err-block last-block-number]
                (let [{:keys [last-processed-block processed-log-indexes]} checkpoint
                  ...
                  (println ">>> EVENTS FROM: " {:last-processed-block last-processed-block :from-block from-block})
                  (println ">>> REPLAY-FROM: " (max (+ 1 last-processed-block) from-block 0))
                  (if skip-past-events-replay?
                    ...
                    (smart-contracts/replay-past-events-in-order
                      events
                      dispatch
                      {:from-block (max (+ 1 last-processed-block) from-block 0)
