...
(defn update-job-role-search
  ...
  (let [job-role-entry {:job-id (:job/id job) :start-date start-date :status status}]
    (update-in acc [user-address role] #(conj (or %1 []) job-role-entry))))

(defn update-jobs [acc {:keys [job]}]
  (update-in acc [(:job/id job)] #(merge %1 job)))
