...
(defn create
  ...
  (let [last-block-number (last-block-number)
        past-event-filter
        (web3-eth/filter @web3 {} {:from-block 0 :to-block (dec last-block-number)})
        watch-event-filter
        (web3-eth/filter @web3 {} "latest")
        event-channel (chan 1)
        stop-channel (chan 1)
        past-events-channel (get-events past-event-filter)
        watched-events-channel (watch-events watch-event-filter)
        finished? (atom false)]
      (go
        ...
        (close! event-channel)
        (stop-watching! watch-event-filter))
