   ...
   (let [provides-web3? (boolean (aget js/window "web3"))
     ...
     (merge
      {:db db
       ...
       :dispatch-interval-n [{:dispatch [:load-conversion-rates]
                              :ms 6000
                              :db-path [:load-all-conversion-rates-interval]}
                             {:dispatch [:blockchain/set-accounts]
                              :ms 6000
                              :db-path [:load-all-conversion-rates-interval]}]}
...
(reg-event-db
 :blockchain/set-account
 interceptors
 (fn [db [account balance]]
   (assoc-in db [:app/users account] {:user/balance balance})))

(reg-event-db
 :blockchain/set-accounts
 interceptors
 (fn [db]
   (mapv #(web3-eth/get-balance (:web3 db) % (fn [error balance]
                                               (dispatch [:blockchain/set-account % balance])))
         (web3-eth/accounts (:web3 db)))   
   (assoc db :app/users {})))
