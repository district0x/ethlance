(ns ethlance.server.model.comment-test
  "Unit tests for the comment models."
  (:require
   ...
   [ethlance.server.model.comment :as comment]
...
(deftest-database main-job-model {}
  ...
  (testing "Create new Comments, get new comments"
    (is (= (count (comment/work-contract-comment-listing 0 0)) 0))

    (comment/create-work-contract-comment!
     {:job/index 0
      :work-contract/index 0
      :comment/index 0
      :comment/revision 0
    ...
    (is (= (count (comment/work-contract-comment-listing 0 0)) 1))

    (let [comment-data (-> (comment/work-contract-comment-listing 0 0) first)]
      (is (= (:comment/text comment-data) "Looking forward to starting!"))
      (is (= (:comment/user-type comment-data) ::enum.user-type/candidate)))

    ;; Creating a revision to a previous comment
    (comment/create-work-contract-comment!
     {:job/index 0
      :work-contract/index 0
      :comment/index 0
    ...
    (let [comment-data (-> (comment/work-contract-comment-listing 0 0) first)]
      (is (= (:comment/text comment-data) "Looking forward to starting! You can reach me at blah blah."))
      (is (= (:comment/user-type comment-data) ::enum.user-type/candidate)))

    ;; Should only grab the latest revision for each comment index.
    (is (= (count (comment/work-contract-comment-listing 0 0)) 1)))
  ...
  (testing "Creating and Retrieving Invoice Comments"
    (is (= (count (comment/invoice-comment-listing 0 0 0)) 0))

    (comment/create-invoice-comment!
     {:job/index 0
      :work-contract/index 0
      :invoice/index 0
      :comment/index 0
    ...
    (is (= (count (comment/invoice-comment-listing 0 0 0)) 1))

    (let [comment-data (-> (comment/invoice-comment-listing 0 0 0) first)]
      (is (= (:comment/text comment-data) "Here's my first invoice"))
      (is (= (:comment/user-type comment-data) ::enum.user-type/candidate)))
    ...
    (let [comment-data (-> (comment/invoice-comment-listing 0 0 0) first)]
      (is (= (:comment/text comment-data) "Edited Comment."))
      (is (= (:comment/user-type comment-data) ::enum.user-type/candidate)))
  ...
  (testing "Creating and Retrieving Dispute Comments"
    (is (= (count (comment/dispute-comment-listing 0 0 0)) 0))

    (comment/create-dispute-comment!
     {:job/index 0
      :work-contract/index 0
      :dispute/index 0
      :comment/index 0
    ...
    (is (= (count (comment/dispute-comment-listing 0 0 0)) 1))

    (let [comment-data (-> (comment/dispute-comment-listing 0 0 0) first)]
      (is (= (:comment/text comment-data) "Here's my first dispute"))
      (is (= (:comment/user-type comment-data) ::enum.user-type/candidate)))
