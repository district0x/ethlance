...
* Requirements
** Job
   - A Job (EthlanceJobProducer) is a smart contract created and updated by an Employer.
     - A Job has many Job Contracts (EthlanceJob)
       - A Job sets the max number of Job Contracts
     - A Job can have only one arbiter
     - A Job requires only one arbiter to 'start' the Job.
   - A Job Contract is a smart contract created by an Employer, or a
     Candidate that has been 'requested'.
     - A Job Contract can be created when a Requested Candidate is
...
** Class Diagram
   ...
   class EthlanceRegistry << (d,violet) Database Contract >> {
     ...
     job_wagon_listing : array[address(EthlanceJobWagon)]
     ...
     -- Job Wagon Database Methods --
     +addJobWagon(eth_address : address, contract_address : address) : uint
     +getJobWagonByIndex(index : uint) : address
     +getJobWagonCount() : uint
   }
   ...
   class JobWagonMetahash << (I,blue) IPFS >> {
   ...
   EthlanceJobWagon "1" *-- "1" JobWagonMetahash
   class EthlanceJobWagon << (f,green) Forwarded Contract >> {
     ...
     metahash : string{JobWagonMetahash}
     reward_value : float

     -createJobWorker(_jobWagon: EthlanceJobWagon) : _worker_id : uint;
     -removeJobWorker(...);
     +getJobWorkerCount();
     +getJobWorkerByIndex();
   }
   ...
   EthlanceJobWorker "*" *--* "1" EthlanceJobWagon
   EthlanceJobWorker "1" *-- "*" EthlanceJobInvoice
   EthlanceJobWorker "1" *-- "*" EthlanceJobDispute
   class EthlanceJobWorker << (f,green) Forwarded Contract >> {
     wagon_instance : EthlanceJobWagon
   ...
   class EthlanceJobInvoice << Contract >> {
     job_instance: address<EthlanceJobWorker>
     date_created: uint{DateTime}
   ...
   EthlanceJobFactory "1" *-- "*" EthlanceJobWagon
